...
int main() {
    ...
    char* general_const=new char[strlen(&general[0])+1];
    strcpy(general_const, &general[0]);
    Character my_character;
    my_character.name = new char[strlen(&name[0])+1];
    my_character.lands->name = new char[strlen(&land[0]+1)];

    strcpy(my_character.name , &name[0]);
    strcpy(my_character.lands->name, &land[0]); 
    ...
    while (choice != 6) {
        ...
        case 1: {
            ...
            cout << "You've talked with your general " << general_const << "." << endl;

            round++;
            endOfTurn(my_character ,round);
            break;
        }
        case 2: {
            listLands(charList);
            ...
            ///// FILL HERE /////
            Character attackSide=charList.getLandOwner(&land[0]);
            if(my_character.manpower < attackSide.manpower) //lose the war
            {
                cout << "You've lost the battle and " << my_character.manpower << " manpower." << endl;
                my_character.manpower=0;
            }
            else
            {
                Land * added=new Land;
                while(attackSide.lands->next)
                {
                    if(attackSide.lands->name=&land[0])
                    {
                        added=attackSide.lands;
                    }
                    attackSide.lands=attackSide.lands->next;
                }
                my_character.addLand(*added);
                attackSide.removeLand(*added);
                my_character.manpower-= attackSide.manpower;
                attackSide.manpower=0;
        ...
        case 3:
            ...
            int cost;
            cost=order*5;
            if(my_character.gold >= cost)
            {
                my_character.gold -= cost;
                my_character.manpower += order;
                cout << "Order successful. You have " << my_character.manpower << " manpower." << endl;
            } 
            else
            {
...
void readData(string fileName, CharacterList& charList) {
    ...
    if (dataFile.is_open()) {
        ...
        while (dataFile >> word) {
            ...
            Character newChar(&name[0], manpower , money, numOfLands);
            ...
            for (int i = 0; i < numOfLands; i++) {
                ...
                ///// FILL HERE ///// 
                Land newLand(&landName[0], &holding[0]);
                newChar.lands= &newLand;
                newChar.lands= newChar.lands->next ;
...
void listCharacters(CharacterList& charList) {
    ...
    for(int i=0; i< charList.size ; i++ ){  
        cout<< charList.list[i].name << endl;
    }
    ///// FILL HERE /////
...
void listLands(CharacterList& charList) {
   ...
   for (int i = 0; i < charList.size ; i++)
   {
       for(int j=0; j< charList.list[i].numOfLands ; j++)
       cout << charList.list[i].lands->name  << " " << charList.list[i].lands->holding<< " owned by " <<  charList.list[i].name << endl;
       charList.list[i].lands= charList.list[i].lands->next;
   }
...
bool endOfTurn(Character& player, int round) { //return a bool value that indicates if the game is over or not.
    ...
    ///// FILL HERE /////
    player.getTaxes();
    player.gold=player.gold-player.manpower;
    if(player.numOfLands==0)
    {
    ...
    {
        player.removeLand( *player.lands);
        cout << "You lost one of your lands to rebellions since you don't have enough army." << endl;
        
    }
    if(player.gold<player.manpower)
    {
        int runaway;
        runaway=player.manpower-player.gold;
        player.manpower=player.manpower-runaway;
        cout << runaway << " soldiers run away from your army because you don't have enough gold to feed them." << endl;
        
    }
    
    cout << "Turn " << round << ": " << " " << player.name << " has " << player.numOfLands
        << " land(s), " << player.manpower << " manpower and " << player.gold << " golds." << endl;
    return true;
